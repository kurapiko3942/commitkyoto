...
export function OptimalRouteCard({ 
   ...
   nearbyStartStops.forEach(startStop => {
     nearbyEndStops.forEach(endStop => {
       ...
       availableBuses.forEach(bus => {
         ...
         // バスでの所要時間を計算
         const relevantStopTimes = stopTimes.filter(st => 
           st.trip_id === bus.vehicle?.trip?.trip_id
         ...
         const busTime = startStopTime && endStopTime ? 
           calculateTimeDifference(startStopTime.arrival_time, endStopTime.arrival_time) :
           20; // デフォルト値
 ...
 const sortRoutes = (routes: RouteOption[], sortBy: 'time' | 'fare' | 'transfers'): RouteOption[] => {
 ...
 return (
   <div className="bg-neutral-800 p-4 rounded-lg text-white">
     <h3 className="font-bold text-lg mb-2">{origin.name}から{destination.name}までの推奨ルート</h3>
       ...
       <div className="border-l-2 border-green-500 pl-3">
         ...
         <div className="text-sm text-gray-400">
           ...
           {bestRoute.bus.vehicle?.position?.speed !== undefined && (
             <p>現在速度: {Math.round(bestRoute.bus.vehicle.position.speed * 3.6)}km/h</p>
           )}
         </div>
       </div>
