...
export async function GET() {
  try {
    ...
    // URLを構築
    const url = GTFS_STATIC_URL.replace(
    ...
    try {
      ...
      // 各ファイルを型付きで並行解析
      const [routes, stops, trips, stopTimes, fareAttributes, fareRules] =
        await Promise.all([
          parseFile<GTFSRoute>("routes.txt"), //routeId
          parseFile<GTFSStop>("stops.txt"), //
          parseFile<GTFSTrip>("trips.txt"), //tripId
